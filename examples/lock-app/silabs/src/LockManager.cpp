 ...
 */
                                ...
                                OperationErrorEnum & err)
 {
     ...
     for (uint32_t userIndex = 1; userIndex <= kMaxUsers; userIndex++)
     {
         ...
         {
             ChipLogError(Zcl, "Unable to get the user - internal error [endpointId=%d,userIndex=%lu]", endpointId,
                          userIndex);
             return false;
         }
         ...
         for (uint32_t userCredentialIndex = 0; userCredentialIndex < user.credentials.size(); userCredentialIndex++)
         {
             ...
             if (user.credentials.data()[userCredentialIndex].credentialType == CredentialTypeEnum::kPin)
             {
                 ...
                 if (!GetCredential(endpointId, user.credentials.data()[userCredentialIndex].credentialIndex, user.credentials.data()[userCredentialIndex].credentialType,
                                    credential))
                 {
